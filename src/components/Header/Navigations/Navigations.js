        ...
        <MobileContext.Consumer>
            {
                ...
                <div className="mr-6 ml-6 flex items-center justify-between text-pd-accent-blue w-full dark:text-pd-dark-accent-blue">
                    {this.meta.map(nav => {return <Link key={nav.href} to={nav.href} className="cursor-pointer material-icons-round md-24">{nav.icon}</Link>;})}
                    <Notification />
                </div>
            </div> :
            <div className="font-medium font-pd-sans-body flex items-center justify-evenly w-full text-pd-accent-blue dark:text-pd-dark-accent-blue">
                {
                    (onMobileScreen) ? this.meta.map(nav => {return <Link key={nav.href} to={nav.href} className="cursor-pointer material-icons-round">{nav.icon}</Link>;}): this.meta.map(nav => {return <Link key={nav.href} to={nav.href} className="cursor-pointer underline text-base">{nav.link}</Link>;})  
                }
            <Notification />
            </div>
            )}
